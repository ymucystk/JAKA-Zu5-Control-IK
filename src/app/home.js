...
const joint_pos = {
  ...
  j1:new THREE.Vector3(0,0,0),
  j2:new THREE.Vector3(0,0.1201,0),
  j3:new THREE.Vector3(0,0.43,0),
  j4:new THREE.Vector3(0,0.36852,0),
  j5:new THREE.Vector3(0.114001,0.113502,0),
  j6:new THREE.Vector3(0,0,0),
  j7:new THREE.Vector3(0,0,0.15),
}
...
//    console.log("Target Update",do_target_update)
  ...
  React.useEffect(() => {
    if (rendered) {
      ...
      set_wrist_rot({x:round(toAngle(w_euler.x)),y:round(toAngle(w_euler.y)),z:round(toAngle(w_euler.z))})
  ...
  const get_p21_pos = ()=>{
    return quaternionToRotation(get_j5_quaternion(),{x:0,y:0,z:p15_16_len})
  }
  ...
  const quaternionToRotation = (q,v)=>{
    return new THREE.Vector3(v.x, v.y, v.z).applyQuaternion(q);
  }
  ...
  const target_update = ()=>{
    ...
    const direction = (toAngle(z_vec_base.angleTo(xz_vector)))*dir_sign1
    if(isNaN(direction)){
    ...
    const angle = (toAngle(y_vec_base.angleTo(y_vector)))*dir_sign2*(Math.abs(direction)>90?-1:1)
    if(isNaN(angle)){
      console.log("p21_pos 指定可能範囲外！")
      set_dsp_message("p21_pos 指定可能範囲外！")
      return
  ...
  const target15_update = (target15,wrist_direction,wrist_angle)=>{
    ...
    let wk_j1_rotate = normalize180((direction_t15 - (90 - kakudo_t15)))
    ...
    const wk_j5_kakudo = (toAngle(p14_extension_pos.angleTo(get_p21_pos())))
    ...
    let wk_j4_rotate = normalize180((((wk_j2_rotate + wk_j3_rotate) * -1) + wk_j4_rotate_sabun))
    if(isNaN(wk_j4_rotate)){
      console.log("wk_j4_rotate 指定可能範囲外！")
      dsp_message = "wk_j4_rotate 指定可能範囲外！"
    }
    if(dsp_message  === ""){
      const wk_j6_rotate_value = normalize180(wk_j6_rotate + tool_rotate)
      ...
      if(round(sabun_distance) <= 0){
        ...
        set_j1_rotate(round(wk_j1_rotate))
        set_j2_rotate(round(wk_j2_rotate))
        set_j3_rotate(round(wk_j3_rotate))
        set_j4_rotate(round(wk_j4_rotate))
        set_j5_rotate(round(wk_j5_rotate))
        set_j6_rotate(round(wk_j6_rotate_value))
...
const Assets = (props)=>{
  ...
  return (
    <a-assets>
      {/*Model*/}
      <a-asset-items id="base" src={`${path}JAKA_Zu_5_BASE.gltf`} ></a-asset-items>
      <a-asset-items id="j1" src={`${path}JAKA_Zu_5_J1.gltf`} ></a-asset-items>
      <a-asset-items id="j2" src={`${path}JAKA_Zu_5_J2.gltf`} ></a-asset-items>
      <a-asset-items id="j3" src={`${path}JAKA_Zu_5_J3.gltf`} ></a-asset-items>
      <a-asset-items id="j4" src={`${path}JAKA_Zu_5_J4.gltf`} ></a-asset-items>
      <a-asset-items id="j5" src={`${path}JAKA_Zu_5_J5.gltf`} ></a-asset-items>
      <a-asset-items id="j6" src={`${path}JAKA_Zu_5_J6.gltf`} ></a-asset-items>
    </a-assets>
  )
}
